...
class EvalCallback():
    def __init__(self, net, input_shape, num_classes, image_ids, dataset_path, log_dir, cuda, \
        ...
        self.image_ids = image_ids  # 关键修改：直接使用原始行数据，不做split处理
        ...
        # 关键修改：移除split()，保留完整文件名（包括空格和括号）
        self.image_ids = [image_id.strip() for image_id in image_ids]  # 仅去除首尾空白符
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #-------------------------------#
                # 关键修改：使用完整文件名拼接路径
                image_path = os.path.join(self.dataset_path, "VOC2007/JPEGImages", f"{image_id}.jpg")
